#from https://github.com/mlfoundations/open_lm/blob/main/open_lm/data.py

# NO Changes needed Yet for Interleaved Embeddings
...
def preprocess_json_text_llama(text):
    text = json.loads(text.decode())
    text = [proc_token_llama(x) for x in text]
    return text
...
def get_wds_dataset(args, is_train, epoch=0, floor=False, tokenizer=None, data_key="json", force_num_samples=-1):
    ...
    for ii, input_shards in enumerate(input_shards_):
        ...
        if data_key == "json":
            preprocess_json_text = preprocess_json_text_llama # if args.vocab_size == 50432 else preprocess_json_text_tiktoken
            pipeline.extend([
